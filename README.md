...
| **[residency-guard](skills/residency-guard/SKILL.md)** | Audits whether a data-residency commitment actually holds at every runtime path — cross-region failover/DR targets, CDN/edge caching, read replicas added later for latency, queue/consumer region pinning, third-party SDKs whose own infrastructure crosses the boundary — not just wherever the primary database lives | "is our data actually staying in the EU", "does failover break our residency promise", "audit data residency enforcement", "can you prove this never left region X" |
...
| You've committed to keeping data in a region/jurisdiction and aren't sure failover, CDN caching, or a vendor SDK still honors that | [residency-guard](skills/residency-guard/SKILL.md) |
...
> **Tip:** start with one or two. Skills work best when each one clearly owns its trigger; copying all twenty-nine at once is fine, but if you only want the security review, just take `security-sweep`.
...
[trail-guard](examples/trail-guard.md),
[proration-guard](examples/proration-guard.md), and
[residency-guard](examples/residency-guard.md) — every skill in the pack has one.
